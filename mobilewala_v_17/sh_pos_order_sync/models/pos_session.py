...
class PosSession(models.Model):
    ...
    def search_session(self):
        sessions = self.search([('state','in',['opened','opening_control'])])
        data = []
        if sessions:
            for each_session in sessions:
                if each_session and each_session.config_id and (each_session.config_id.user_type == 'receive' or each_session.config_id.user_type == 'both'):
                    data.append({'session_id':each_session.id,'session_name':each_session.name,'config_id':each_session.config_id.id,'config_name':each_session.config_id.name,'sh_nick_name':each_session.config_id.sh_nick_name})
        return data
    
    def _pos_data_process(self, loaded_data):
        super()._pos_data_process(loaded_data)
        loaded_data['sh_config_data'] = self.env['pos.config'].search_read([], ['name', 'sh_nick_name'])

    def get_order_fields(self):
        sh_order_fields = super().get_order_fields()
        sh_order_fields.append('sender_config')
        return sh_order_fields
